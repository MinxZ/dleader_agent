...
# """)
    # for i in range(1, 2):
    # for file_path in ['pdf/400256000_22800AMX00681_F100_1.pdf']:
    # for year in range(2010, 2026):
    # for year in [2011]:
    #     log, result = agent.go(f"""I would like to create a list of small-molecule compound drugs newly approved for {year}, from kegg_{year}.csv, can help me to make the list of drugs and save to kegg_small_molecules_{year}.csv? with column Date,Drug_ID,ATC_Code,Generic_Name,Brand_Name,Company,Type""")
    # for year in [2011]:
    log, result = agent.go(f"""I would like to create a list of small-molecule compound drugs kegg_small_molecules_2010-2025.csv? with column Date,Drug_ID,ATC_Code,Generic_Name,Brand_Name,Company,Type, can you merge them into one file""")

        # print("📋 Agent Response:")
        # print(log, result)
        # time.sleep(60)
        # with open('drug_list24_fda_log.txt', 'w') as f:
        #     f.write(str(log))
        # with open('drug_list24_fda_result.txt', 'w') as f:
        #     f.write(str(result))
