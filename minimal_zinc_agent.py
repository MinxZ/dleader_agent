...
# """)
    ...
    log, result = agent.go(f"""summarize the unique features of zinc database (250k_rndm_zinc_drugs_clean.smi)
""")

    print("📋 Agent Response:")
    print(log, result)
    with open('zinc_local_log.txt', 'w') as f:
        f.write(str(log))
    with open('zinc_local_result.txt', 'w') as f:
        f.write(str(result))
