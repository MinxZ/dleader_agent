...
def extract_patent_images(html_path, output_dir=None):
    """
    从专利HTML文件中提取图片URL信息
    提取所有<meta itemprop="full" content="...">中以https://patentimages.storage.googleapis.com/开头的URL
    
    Args:
        html_path: HTML文件路径
        output_dir: 输出目录，如果不指定则使用HTML文件所在目录
    ...
    # 查找所有meta标签，itemprop="full"
    meta_tags = soup.find_all("meta", {"itemprop": "full"})
    ...
    for meta in meta_tags:
        ...
        if content.startswith("https://patentimages.storage.googleapis.com/"):
            # 提取文件名
            ...
            urls_only.append(content)
    
    # 生成输出文件名
    html_filename = Path(html_path).stem
    json_output = output_dir / f"{html_filename}_images.json"
    txt_output = output_dir / f"{html_filename}_urls.txt"
    ...
    print(f"提取到 {len(image_data)} 个图片URL")
    print(f"JSON文件: {json_output}")
    print(f"文本文件: {txt_output}")
...
if __name__ == "__main__":
    ...
    # 提取图片URL
    images = extract_patent_images(html_path)
