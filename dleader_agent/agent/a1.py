...
from dleader_agent.env_desc import data_lake_dict, library_content_dict
from dleader_agent.llm import SourceType, get_llm
...
class A1:
    def __init__(
        ...
        """Initialize the dleader_agent agent.
        ...
        benchmark_dir = os.path.join(path, "dleader_agent_data", "benchmark")
        data_lake_dir = os.path.join(path, "dleader_agent_data", "data_lake")

        # Create the dleader_agent_data directory structure
        ...
        if download_data_lake:
            ...
            check_and_download_s3_files(
                s3_bucket_url="https://dleader_agent-release.s3.amazonaws.com",
        ...
        self.path = os.path.join(path, "dleader_agent_data")
    ...
    def add_tool(self, api):
        ...
        try:
            ...
            if not hasattr(builtins, "_dleader_agent_custom_functions"):
                builtins._dleader_agent_custom_functions = {}
            builtins._dleader_agent_custom_functions[schema["name"]] = api
    ...
    def add_mcp(self, config_path: str | Path = "./tutorials/examples/mcp_config.yaml") -> None:
        ...
        the dleader_agent agent system. Each MCP server is loaded as an independent module
        ...
        for server_name, server_meta in mcp_servers.items():
            ...
            for tool_meta in tools_config:
                if isinstance(tool_meta, dict) and "dleader_agent_name" in tool_meta:
                    # Manual tool definition
                    tool_name = tool_meta.get("dleader_agent_name")
    ...
    def remove_custom_tool(self, name):
        ...
        if hasattr(builtins, "_dleader_agent_custom_functions") and name in builtins._dleader_agent_custom_functions:
            del builtins._dleader_agent_custom_functions[name]
...
- DO NOT use emojis (like 😀, ✓, ✗, 📊, etc.) in the final report. Keep the report professional and text-only.
- DO NOT use checkbox syntax like [x] or [ ] in the final report.
  ...
  - Bash scripts and commands: Use the #!BASH marker in your execute block for both simple commands and complex shell scripts with variables, loops, conditionals, etc.
    ...
    def configure(self, self_critic=False, test_time_scale_round=0):
        ...
        def generate(state: AgentState) -> AgentState:
            ...
            if answer_match:
                ...
                error_count = sum(
                    1 for m in state["messages"] if isinstance(m, HumanMessage) and "there are no tags" in m.content.lower()
                )
    ...
    def update_system_prompt_with_selected_resources(self, selected_resources):
        ...
        for tool in selected_resources["tools"]:
            ...
            if isinstance(tool, dict):
                ...
                if not module_name:
                    module_name = "dleader_agent.tool.scRNA_tools"  # Default to scRNA_tools as a fallback
                    tool["module"] = module_name
            else:
    ...
    def _inject_custom_functions_to_repl(self):
        ...
        if hasattr(self, "_custom_functions") and self._custom_functions:
            ...
            from dleader_agent.tool.support_tools import _persistent_namespace
            ...
            if not hasattr(builtins, "_dleader_agent_custom_functions"):
                builtins._dleader_agent_custom_functions = {}
            builtins._dleader_agent_custom_functions.update(self._custom_functions)

    def create_mcp_server(self, tool_modules=None):
        """
        Create an MCP server object that exposes internal dleader_agent tools.
        ...
        mcp = FastMCP("dleader_agentTools")
            ...
            try:
                ...
                for tool_schema in module_tools:
                    ...
                    try:
                        ...
                        wrapper_func = self._generate_mcp_wrapper_from_dleader_agent_schema(
    ...
    def _generate_mcp_wrapper_from_dleader_agent_schema(self, original_func, func_name, required_params, optional_params):
        """Generate wrapper function based on dleader_agent schema format."""
        import inspect
