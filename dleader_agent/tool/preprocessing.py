"""
Data Preprocessing Tools for dleader_agent
...
def load_and_inspect_data(file_path, sep=",", encoding="utf-8"):
    ...
    try:
        ...
        return {"data": data, "inspection": inspection}
...
def detect_data_quality_issues(data, missing_threshold=0.5, high_cardinality_threshold=100, correlation_threshold=0.95):
    ...
    for col in categorical_cols:
        ...
        if unique_count > high_cardinality_threshold:
            issues["high_cardinality_categorical"].append({"column": col, "unique_count": unique_count})
            ...
            for row in upper_triangle.index:
                if upper_triangle.loc[row, col] > correlation_threshold:
                    high_corr_pairs.append(
                        {"feature1": row, "feature2": col, "correlation": upper_triangle.loc[row, col]}
                    )
    ...
    for col in data.select_dtypes(include=["object"]).columns:
        try:
            ...
            issues["data_type_issues"].append({"column": col, "issue": "Numeric data stored as object"})
    ...
    for col in numeric_data.columns:
        ...
        if len(outliers) > 0:
            issues["outlier_columns"].append(
                {"column": col, "outlier_count": len(outliers), "outlier_percentage": len(outliers) / len(data) * 100}
            )
...
def create_data_preprocessing_pipeline(
    ...
    if len(numeric_cols) > 0:
        ...
        processed_data, scaler = normalize_data(processed_data, columns=numeric_cols, method=normalization_method)
