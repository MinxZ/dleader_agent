...
cleanup_old_installations() {
    ...
    local old_afs_dir="/afs/cs.stanford.edu/u/$(whoami)/dleader_agent_tools"
    ...
    local old_home_dir="$HOME/dleader_agent_tools"
...
if [ -n "$dleader_agent_TOOLS_DIR" ]; then
    TOOLS_DIR="$dleader_agent_TOOLS_DIR"
else
    TOOLS_DIR="$(pwd)/dleader_agent_tools"
fi
...
echo "# Added by dleader_agent setup" >> "$TOOLS_DIR/setup_path.sh"
    ...
    if [ -n "$profile_file" ]; then
        ...
        if [ -w "$profile_file" ]; then
            ...
            if ! grep -q "export PATH=\"$TOOLS_DIR/bin:\$PATH\"" "$profile_file"; then
                # Remove any old dleader_agent_tools paths first
                if grep -q "dleader_agent_tools/bin" "$profile_file"; then
                    echo -e "${YELLOW}Removing old dleader_agent_tools paths from $profile_file...${NC}"
                    sed -i '/dleader_agent_tools\/bin/d' "$profile_file"
                fi

                echo "" >> "$profile_file"
                echo "# Added by dleader_agent setup" >> "$profile_file"
...
install_cli_tools() {
    ...
    if [ -n "$dleader_agent_AUTO_INSTALL" ]; then
        install_all_tools 1
...
if [ "$1" = "--help" ] || [ "$1" = "-h" ]; then
    ...
    # Set dleader_agent_AUTO_INSTALL if it's not already set
    export dleader_agent_AUTO_INSTALL=1
    ...
    exit 0
elif [ -n "$dleader_agent_AUTO_INSTALL" ]; then
    # Run in auto-install mode without showing the menu
    install_all_tools 1
