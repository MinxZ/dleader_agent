...
# BioAgentOS - dleader_agent Environment Setup Script
...
DEFAULT_TOOLS_DIR="$(pwd)/dleader_agent_tools"
TOOLS_DIR=""

echo -e "${YELLOW}=== dleader_agent Environment Setup ===${NC}"
...
install_cli_tools() {
    ...
    export dleader_agent_TOOLS_DIR="$TOOLS_DIR"
    ...
    if [ $? -eq 0 ]; then
        ...
        echo "# Added by dleader_agent setup" >> setup_path.sh
    ...
    unset dleader_agent_AUTO_INSTALL
    unset dleader_agent_TOOLS_DIR
}
...
main() {
    ...
    conda env create -n dleader_agent_e1 -f environment.yml
    ...
    if command -v micromamba &> /dev/null; then
        ...
        micromamba activate dleader_agent_e1
    else
        eval "$(conda shell.bash hook)"
        conda activate dleader_agent_e1
    fi
    handle_error $? "Failed to activate dleader_agent_e1 environment."
    ...
    # Setup completed
    echo -e "\n${GREEN}=== dleader_agent Environment Setup Completed! ===${NC}"
    echo -e "You can now run the example analysis with: ${YELLOW}python bio_analysis_example.py${NC}"
    echo -e "To activate this environment in the future, run: ${YELLOW}conda activate dleader_agent_e1${NC}"
    ...
    PATH=$(echo $PATH | tr ':' '\n' | grep -v "dleader_agent_tools/bin" | tr '\n' ':' | sed 's/:$//')
    export PATH="$(pwd)/dleader_agent_tools/bin:$PATH"
}
